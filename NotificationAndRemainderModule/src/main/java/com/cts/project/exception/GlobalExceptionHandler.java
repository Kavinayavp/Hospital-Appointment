...
public class GlobalExceptionHandler {
	...
	public Map<String, String> handleValidationErrors(MethodArgumentNotValidException ex) {
		...
		ex.getBindingResult().getFieldErrors().forEach(error -> {
			...
			LOGGER.error("❌ Validation Error - {}: {}", fieldName, errorMessage); // ✅ Logs validation errors in
																					// readable format
		});
	...
	public Map<String, String> handleConstraintViolations(ConstraintViolationException ex) {
		...
		ex.getConstraintViolations().forEach(violation -> {
			...
			LOGGER.error("❌ Validation Error - {}: {}", field, message); // ✅ Logs Hibernate constraint violations
																			// clearly
		});
