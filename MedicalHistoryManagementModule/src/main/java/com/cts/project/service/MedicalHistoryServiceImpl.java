...
//        	history.setHistoryId(dto.getHistoryId());
	...
	private MedicalHistoryDTO mapToDTO(MedicalHistory history) {
		return MedicalHistoryDTO.builder().historyId(history.getHistoryId()).patientId(history.getPatientId())
				.doctorId(history.getDoctorId()).doctorName(history.getDoctorName())
				.patientName(history.getPatientName()).illness(history.getIllness()).treatment(history.getTreatment())
				.visitDate(history.getVisitDate()).prescription(history.getPrescription()).build();
	}

	private MedicalHistory mapToEntity(MedicalHistoryDTO dto) {
		return MedicalHistory.builder().historyId(dto.getHistoryId()).patientId(dto.getPatientId())
				.doctorId(dto.getDoctorId()).doctorName(dto.getDoctorName()).patientName(dto.getPatientName())
				.illness(dto.getIllness()).treatment(dto.getTreatment()).visitDate(dto.getVisitDate())
				.prescription(dto.getPrescription()).build();
	}
